...
const { Profile, AppClientTokenService, UuAppWorkspace, UuAppWorkspaceError } =
  require("uu_appg01_server").Workspace;
const { UriBuilder } = require("uu_appg01_server").Uri;
const { LoggerFactory } = require("uu_appg01_server").Logging;
const { AppClient } = require("uu_appg01_server");
const Errors = require("../api/errors/library-main-error.js");
const { Schemas, Profiles, Library } = require("./constants.js");
...
class LibraryMainAbl {
  ...
  async init(uri, dtoIn, session) {
    ...
    // HDS 2
    const schemas = ["libraryMain", "book", "author", "location"];
    ...
    if (dtoIn.uuBtLocationUri) {
      ...
      const createAwscDtoIn = {
        name: "UuLibrary",
        typeCode: "uu-library-maing01",
        location: location,
        uuAppWorkspaceUri: baseUri,
      };

      const awscCreateUri = uuBtUriBuilder.setUseCase("uuAwsc/create").toUri();
      const appClientToken = await AppClientTokenService.createToken(
        uri,
        uuBtBaseUri,
      );
      const callOpts = AppClientTokenService.setToken(
        { session },
        appClientToken,
      );
      ...
      try {
        ...
        if (e.code.includes("applicationIsAlreadyConnected") && e.paramMap.id) {
          ...
          throw new Errors.Init.CreateAwscFailed(
            { uuAppErrorMap },
            { location: dtoIn.uuBtLocationUri },
            e,
          );
        }
      }

      const artifactUri = uuBtUriBuilder
        .setUseCase(null)
        .clearParameters()
        .setParameter("id", awscId)
        .toUri();
  ...
  async loadBasicData(uri, session, uuAppErrorMap = {}) {
    // HDS 1
    const dtoOut = await UuAppWorkspace.loadBasicData(
      uri,
      session,
      uuAppErrorMap,
    );
